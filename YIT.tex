...
\]
li pona tawa sama.}
\tran{Deutsch (ISO 639-3 deu)}{Lemma von Yoneda}{
Es seien $\mathcal C$ eine kleine Kategorie und $F$ ein Funktor von $\mathcal C$ in die Kategorie der Mengen. F\"ur jedes Objekt $X$ der Kategorie $\mathcal C$ ist die Bijektion zwischen der Menge der nat\"urlichen Transformationen $\hom(-,X)\to K$ und der Menge $FX$ wie folgt definiert:
\[
	\Big(\xi : \hom(-,X)\Rightarrow F\Big) \mapsto \xi_X(1_X)
\]
